...
export async function getCombinedBlogPosts(
  ...
  const stagedWebflowPosts = await listStagedWebflowBlogPosts();
...
export async function updateBlogPostPreview({
  notionId,
}: {
  notionId: string;
}) {
  ...
  const webflowAuthorIds = notionAuthors.map((author: string) => {
  ...
  const webflowCategory = blogCategories.items!.find(
  ...
  const cmsItemData = {
    ...
    slug,
    body: html || "",
    "cover-image": coverPhotoUrl,
    "thumbnail-image": thumbnailUrl ?? coverPhotoUrl,
    category: webflowCategory?.id,
    authors: webflowAuthorIds,
